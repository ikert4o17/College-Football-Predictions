...
Important: historical seasons use prior-season Gridiron ratings as the starting
anchor because archived same-season preseason SP+ snapshots are not stored in
this repository. Since the frozen 2026 Balanced Light anchor is stronger, this
backtest should be treated as an upper-bound test of how quickly we should learn,
not a reason to move 2026 ratings more aggressively than history supports.
"""
...
def evaluate_season(season,params,coeff,hfa):
 anchor=anchor_lookup(load(SEASONS[season]["anchor"]));ratings=dict(anchor);gp={t:0 for t in ratings}
 updated=[];static=[];uw=sw=tested=0; buckets={i:{"updated":[],"static":[],"deltas":[]} for i in range(0,13)}
 for g in eligible(load(SEASONS[season]["games"]),season):
...
def evaluate(params,coeff,hfa):
 seasons={str(y):evaluate_season(y,params,coeff,hfa) for y in SEASONS};mi=sum(v["mae_improvement"] for v in seasons.values())/2;ri=sum(v["rmse_improvement"] for v in seasons.values())/2
 return {"params":dict(params),"seasons":seasons,"mean_mae_improvement":round(mi,4),"mean_rmse_improvement":round(ri,4),"improves_mae_and_rmse_both_seasons":all(v["mae_improvement"]>0 and v["rmse_improvement"]>0 for v in seasons.values())}
...
def main():
 ...
 current=evaluate(CURRENT_V1,coeff,hfa);keys=list(GRID);cands=[]
 for vals in itertools.product(*(GRID[k] for k in keys)):cands.append(evaluate(dict(zip(keys,vals)),coeff,hfa))
 valid=[c for c in cands if c["improves_mae_and_rmse_both_seasons"]]
 ...
 best=ranked[0]
 result={"method":"sequential margin-residual update vs static prior-season anchor","caveat":"Historical anchor is weaker/staler than frozen 2026 preseason SP+ + Balanced Light; recommended learning is an upper bound for 2026.","frozen_2026_preseason":"2026 preseason SP+ + Balanced Light (RP 1.25, TT 1.00, QB -0.25, coaching 0.50, cap 4)","calibration":{"rating_gap_coefficient":coeff,"home_field_advantage":hfa},"parameter_combinations":len(cands),"valid_candidates_improving_mae_and_rmse_both_seasons":len(valid),"current_v1":current,"recommended":best,"recommended_learning_schedule":schedule(best["params"]),"top_20":ranked[:20]}
 ...
 print("Candidates:",len(cands)," valid both seasons:",len(valid));print("Recommended:",best["params"]);print("Learning schedule:")
